...
def load_model():
    model_path = Path(__file__).parent / "dish.pkl"  # 跨平台路径
    if not model_path.exists():
        raise FileNotFoundError(f"模型文件不存在: {model_path}")
    
    try:
        # 方案1：优先用 torch.load（更底层，避免 fastai 路径问题）
        with open(model_path, 'rb') as f:
            return torch.load(f, map_location='cpu', pickle_module=pickle)
    except Exception as e:
        # 方案2：明确转换为字符串路径
        try:
            return load_learner(str(model_path))  # 关键：转成字符串
        except Exception as e2:
            raise RuntimeError(f"模型加载失败:\n原始错误: {e}\n备选错误: {e2}")

# 新增：调用 load_model 获取 model 并赋值
try:
    model = load_model()
except Exception as e:
    st.error(f"模型加载失败: {e}")
...
try:
    # 使用 pathlib 处理菜品信息文件路径
    dishes_df = pd.read_excel(DISHES_FILE)
    
    # 构建菜品ID映射，此时 model 已定义
...
# 假设测试图片存在 'test_imgs' 文件夹，命名为 '酱烧豆腐.jpg'
TEST_IMG_PATH = Path(__file__).parent / 'test_imgs/酱烧豆腐.jpg'

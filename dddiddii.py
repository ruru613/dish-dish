...
class PathProcessor:
    @staticmethod
    def convert_windows_path(obj):
        """将WindowsPath对象转换为字符串，避免跨平台问题"""
        if isinstance(obj, Path):
            return str(obj)
        if isinstance(obj, dict):
            return {k: PathProcessor.convert_windows_path(v) for k, v in obj.items()}
        if isinstance(obj, list):
            return [PathProcessor.convert_windows_path(v) for v in obj]
        return obj
    
    @staticmethod
    def fix_path_after_load(learner):
        """加载模型后修复路径问题"""
        # 检查并修复dls中的路径
        if hasattr(learner, 'dls') and hasattr(learner.dls, 'path'):
            learner.dls.path = Path(str(learner.dls.path))
            
        # 检查并修复模型中的其他路径
        if hasattr(learner, 'model') and hasattr(learner.model, 'state_dict'):
            # 这里可以添加更多特定于模型的路径修复逻辑
            pass
...
def load_model():
    ...
    logger.info(f"开始加载模型: {MODEL_PATH} (当前平台: {platform.system()})")
    
    try:
        # 方案1：自定义加载器，处理路径序列化问题
        ...
        # 修复加载后的路径
        if hasattr(model, 'dls') and hasattr(model.dls, 'path'):
            model.dls.path = Path(str(model.dls.path))
        ...
        try:
            # 方案2：使用 torch.load 配合自定义反序列化
            with open(MODEL_PATH, 'rb') as f:
                # 先加载为字典
                model_dict = torch.load(f, map_location='cpu')
                
                # 处理字典中的路径
                model_dict = PathProcessor.convert_windows_path(model_dict)
                ...
                if 'model' in model_dict:
                    learn.model.load_state_dict(model_dict['model'])
                elif 'state_dict' in model_dict:
                    learn.model.load_state_dict(model_dict['state_dict'])
                else:
                    # 尝试直接加载整个模型
                    learn = model_dict
                    
                # 修复路径
                learn = PathProcessor.fix_path_after_load(learn)
            ...
            try:
                # 方案3：使用 load_learner 但添加路径处理
                # 注意：这种方法在跨平台时最容易失败
                model = load_learner(str(MODEL_PATH))
                
                # 修复路径
                model = PathProcessor.fix_path_after_load(model)
...
try:
    ...
    st.error(f"模型加载失败: {e}")
    st.error("请确保模型是在与部署环境相同的操作系统上训练的，或者使用跨平台兼容的方式保存模型。")
    st.stop()
